...
void MyAnalyzer()
{
  ...
  TH1* hDiff_etaC_etaM = new TH1F("hDiff_etaC_etaM","Difference between eta_measured and eta_calculated for a leading energy cluster",80,-0.1,0.1);
  ...
  TH2* hScatter_Delta_eta_Delta_rho=new TH2F("hScatter_Delta_eta_Delta_rho","Scatter plot of Delta_eta versus Delta_rho",120,-0.06,0.06,240,-6.,6.); 
  ...
  for(int j=1;j<=NumInputFiles;j++){
	...
	for( uint genIndex = 0; genIndex < genpart_pt->size(); genIndex++ ) {
	  ...
	  if(genpart_pid->at(genIndex)==22 && genpart_reachedEE->at(genIndex)==1 && genpart_pt->at(genIndex)==35.){
	    ...
	    for(int LIndex=10;LIndex<11;LIndex++){
	      int startIndex=-1;
		...
		Max_cluster2D_rhoC=sqrt(Max_cluster2D_x*Max_cluster2D_x+Max_cluster2D_y*Max_cluster2D_y);
		Max_cluster2D_thetaC=atan(Max_cluster2D_rhoC/Max_cluster2D_z);
		Max_cluster2D_etaC=-log(tan(Max_cluster2D_thetaC/2));
		...
		if(Max_cluster2D_energy!=0){
		  if(Max_cluster2D_eta>0)
		    hDelta_rho_etaPlus->Fill(rhoM_rhoT);
		  if(Max_cluster2D_eta<0)		
		    hDelta_rho_etaMinus->Fill(rhoM_rhoT);
  ...
  // hDelta_eta.at(jx)->Fill(etaM_etaT);
		  ...
		  hDiff_etaC_etaM->Fill(Max_cluster2D_etaC-Max_cluster2D_eta);		
		}
		//if(jx%3==0)
  ...
  hDelta_rho_etaPlus->Write();
  hDelta_rho_etaMinus->Write();

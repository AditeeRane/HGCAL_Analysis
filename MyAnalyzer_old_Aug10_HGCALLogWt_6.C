...
using std::string;
using std::vector;
void MyAnalyzer_old_Aug10_HGCALLogWt_6()
{
  // get file and tree
  ...
  TH1* hEneProfileVsLayers = new TH1F( "hEneProfileVsLayers", "Energy deposition as a function of layers", 50, 0., 50. );
  ...
  int NoMatchingClusToPhoton=0;
  int TotNumOfEvents=0;
  int TotPhotonsOfInterest=0; 
  for(int j=1;j<=NumInputFiles;j++){
    sprintf(fileName,"/eos/cms/store/group/dpg_hgcal/comm_hgcal/aditee/FlatRandomPtGunProducer_test_Aditee_20170719/NTUP/partGun_PDGid22_x20_Pt35.0To35.0_NTUP_%i.root",j);
	...
	for( uint genIndex = 0; genIndex < genpart_pt->size(); genIndex++ ) {
	  ...
	  if(genpart_pid->at(genIndex)==22 && genpart_reachedEE->at(genIndex)==1 && genpart_pt->at(genIndex)==35.){
	    ...
	    int startMultiClusIndex=-1;
	    float MaxMultiClusFound=9999999.;
	    float Max_multiclus_energy=0;

	    // get the index and energy corresponding to maximum energy multicluster
		...
		if(multiclusEneIndex!=startMultiClusIndex && multiclus_energy->at(multiclusEneIndex)<=MaxMultiClusFound){
		  if(multiclus_energy->at(multiclusEneIndex)>Max_multiclus_energy){
		    Max_multiclus_energy=multiclus_energy->at(multiclusEneIndex);
		    startMultiClusIndex=multiclusEneIndex;
		  }
		}
	      }
	    } //end of loop over multiclusEneIndex
	    MaxMultiClusFound=Max_multiclus_energy; 


	    //proceed only if energy of a leading multicluster is above 1 GeV
	    if(Max_multiclus_energy>1.0){


	      //get number of 2D clusters in layer 6 belonging to highest energy multicluster
	      int startIndex=-1;
		...
		if((photon_Eta>0 && cluster2d_eta->at(clusEneIndex)>0) || (photon_Eta<0 && cluster2d_eta->at(clusEneIndex)<0)){
	      ...
	      //Find the maximum energy 2D cluster from layer6 which belongs to leading multicluster
	      double rhoT=0;
	      double rhoM=0;
	      double thetaT=0;
	      double thetaM=0;
  ...
  std::cout<<" TotNumOfEvents "<<TotNumOfEvents<<" TotPhotonsOfInterest "<<TotPhotonsOfInterest<<" NoMatchingClusToPhoton "<<NoMatchingClusToPhoton<<endl;
  ...
  // draw histograms
  TFile *ouputFile = new TFile("BasicPlots_old_Aug10_HGCALLogWt_6.root","RECREATE");
  hMultiPt->Write();

...
math::XYZPoint HGCalImagingAlgo::calculatePosition(std::vector<KDNode> &v){
  ...
  bool haloOnlyCluster = false;
  //*AR-170714-This additional loop is introduced while trying log energy weighing.
  //*AR-170714- Gets sum of energies of all cells within a cluster
  for (unsigned int i = 0; i < v_size; i++){
    if(!v[i].data.isHalo){
      energy_sum += v[i].data.weight;
    }
  }
  haloOnlyCluster = energy_sum <= 0.;
  //*AR-170714-Apply log energy weighing
  if(!haloOnlyCluster){
    for (unsigned int i = 0; i < v_size; i++){
      if(!v[i].data.isHalo){
	float log_wt=(w_zero + log(v[i].data.weight/energy_sum));
	if(log_wt>0.){	
	  total_weight += log_wt;
	  x += v[i].data.x*log_wt;
	  y += v[i].data.y*log_wt;
